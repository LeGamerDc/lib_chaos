...
func main() {
	var t = read()
	var nav = new(detour.NavMesh)
	nav.MVert = make([]mesh.Vert, len(t.MVert))
	for i, v := range t.MVert {
		nav.MVert[i] = v
	}
	nav.MPoly = make([]detour.Poly, len(t.MTri))
	for i, tr := range t.MTri {
		var v0, v1, v2 = tr.Vs()
		nav.MPoly[i] = detour.Poly{
			Link:    -1,
			VertCnt: 3,
			AreaId:  0,
			Vs:      [6]int32{v0, v1, v2},
	...
	var buf = nav.Svg(t.Max, v0, v1, p.ps)
...
func (p *path) Append(x, y, z float64) {
	p.ps = append(p.ps, mesh.Vert{X: x, Y: y, Z: z})
}

func (p *path) LastPos() mesh.Vert {
	return p.ps[len(p.ps)-1]
}
